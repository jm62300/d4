...
/**
 * @brief EliminatorResolution::selectVarAndPop implementation.
 *
 */
...
/**
 * @brief EliminatorResolution::generateAllResolution implementation.
 *
 */
...
void EliminatorResolution::eliminate(unsigned nbVar,
  ...
  while (!timer.isTimeout() && forgetApplied) {
    forgetApplied = false;

    std::vector<Var> pass;
    while (!timer.isTimeout() && inProcess.size()) {
