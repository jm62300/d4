...
/**
 * @brief Bipartition::separator implementation.
 */
...
/**
 * @brief Bipartition::printHeader implementation.
 */
...
/**
 * @brief Bipartition::printInfo implementation.
 */
...
/**
 * @brief Bipartition::updateInfo implementation.
 */
...
/**
 * @brief Bipartition::initProblem implementation.
 */
...
/**
 * @brief Bipartition::initInputModelVariable implementation.
 */
...
/**
 * @brief Bipartition::setPriority implementation.
 */
...
/**
 * @brief Bipartition::exploitSetOfModelAsPreprocToSpotInput implementation.
 */
...
void Bipartition::compute(Problem& p, WrapperSolver* solver,
                          ...
                          std::ostream& out, bool verbose, const Timer& timer) {
  out << "c \033[1m\033[31mBipartition in progress\033[0m\n";
  std::vector<Lit> areUnit;
  std::vector<Var> newInput;

  // compute the partition.
  while (!timer.isTimeout() && !heuristic->isEmpty()) {
  ...
  if (timer.isTimeout()) {
    out << "c [BIPARTITION] Timeout, number of remaining undefined variables: "
        << heuristic->remainingAssumptions().size() << "\n";
...
bool Bipartition::run(Problem& p, std::vector<Var>& input,
  ...
  while (true) {
    ...
    compute(p, m_solver, input, und, heuristic, mapSelector,
            ...
            optionBipartition.verbose, timer);
