 ...
 */
void ParserDimacs::parse_DIMACS_main(BufferRead& in,
  ...
  unsigned& nbVars = parsedProblem.nbVar;
  std::vector<int>& projected = parsedProblem.projected;
  std::vector<std::vector<int>>& clauses = parsedProblem.clauses;
  ...
  for (;;) {
    ...
    if (in.currentChar() == 'p') {
      ...
      if (in.currentChar() != 'p')
        ...
        in.consumeChar();
        if (in.canConsume("show")) {
          readListIntTerminatedByZero(in, projected);
        } else
          in.skipLine();
      }
    } else {
  ...
  if (!projected.size()) {
    for (unsigned i = 1; i <= nbVars; i++) projected.push_back(i);
  }
}  // parse_DIMACS_main
...
void ParserDimacs::parse_DIMACS(std::string input_stream,
  ...
  parse_DIMACS_main(in, parsedProblem);
}  // parse_DIMACS
