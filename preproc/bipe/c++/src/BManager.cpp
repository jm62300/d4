...
bool BManager::run(po::variables_map& vm, bipe::Problem& pb,
                   ...
                   std::vector<bipe::Gate>& gates, const bipe::Timer& timer) {
  ...
  if (optUseBackbone)
    formula = b.simplifyBackbone(pb, optionBackbone, gates, std::cout,
                                 setOfModels, timer);
  else
    formula = b.simplifyOneCall(pb, "Glucose", 0, gates, std::cout, setOfModels,
                                timer);

  if (formula && optUseDac) {
    bipe::Problem* tmp = formula;
    formula =
        b.simplifyDac(*tmp, optionDac, gates, std::cout, setOfModels, timer);
    delete tmp;
  }
