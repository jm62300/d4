 ...
 */
AndGatesExtractor::AndGatesExtractor(int nbVar)
{
  init(nbVar);
} // constructor
 ...
 */
void AndGatesExtractor::searchAndGates(
    SpecManagerCnf *om,
    std::vector<Var> &vars,
    std::vector<AndGate> &gates)
{
  for(unsigned i = 0 ; i<om->getNbClause() ; i++)
  {
    ...
    for(auto &l : cl)
    {
      std::vector<int> &lbin = om->getVecIdxClauseBin(~l);
      if(cpt > lbin.size() + 1) continue;

      for(auto &idClause : lbin)
      {
        std::vector<Lit> &clBin = om->getClause(idClause);
      ...
      if(cptLinked == cpt)
      {
        ...
        ngate.output = l;        
        for(auto &m : cl) if(m != l) ngate.input.push_back(~m);
        break;
      }      
    }
