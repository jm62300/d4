...
template<class T> class BucketManagerCnfIndex : public BucketManagerCnf<T>
{
  ...
  */
  BucketManagerCnfIndex(SpecManagerCnf &occM,
                        Cache<T> *cache,
                        int mdStore,
  ...
  /**
     ...
     @param[]
  */
  template <typename U, typename W> void *storeData(void *data,
                                                    std::vector<W> &value)
  {
  ...
  */
  inline void storeFormula(std::vector<Var> &component, CachedBucket<T> &b)
  {
    ...
    unsigned int nbOData = m_idxClauses.size() ?
                           this->nbOctetToEncodeInt(m_idxClauses.back() + 1) : 1;
 ...
 fillTheBucket:
    DataInfoCnf di(szData, component.size(), 0, m_idxClauses.size(), nbOVar, 1, nbOData);
