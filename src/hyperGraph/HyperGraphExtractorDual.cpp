 ...
 */
HyperGraphExtractorDual::HyperGraphExtractorDual(
    unsigned nbVar,
    unsigned nbClause
)
{
...
/**
   ...
   @param[in] mappingVar, to map the indices of the edge to the variable.
...
/**
   ...
   @param[in] equivClass, for each variable we give the variable that replaces it.
    ...
    HyperGraph &hypergraph)
{
  ...
  {    
    ...
    for(auto &v : vec)
    {
      ...
      {
        for(auto &idx : om.getVecIdxClauseNotBin(l))
          if(!m_markedClauses[idx])
          {
            m_markedClauses[idx] = true;
            m_unmarkSet.push_back(idx);
            hypergraph[pos++] = idx; size++;            
          }

        for(auto &idx : om.getVecIdxClauseBin(l))
          if(!m_markedClauses[idx])
          {
            m_markedClauses[idx] = true;
            m_unmarkSet.push_back(idx);
