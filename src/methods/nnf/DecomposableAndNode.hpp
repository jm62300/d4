...
template <class T, typename U> class DecomposableAndNode : public Node<T>
{
  ...
  */
                         ...
                         unsigned globalStamp)
  {
    ...
    for(unsigned i = 0 ; i<reinterpret_cast<DecomposableAndNode*>(node)->size ; i++)
      reinterpret_cast<void (**)(Node<T> *, void (**func)(), unsigned)>(func)
          [(reinterpret_cast<DecomposableAndNode*>(node)->sons[i])->header.typeNode](
              reinterpret_cast<DecomposableAndNode*>(node)->sons[i], func, globalStamp);
  } // destructor
                           ...
                           unsigned globalStamp)
  {
    ...
    for(unsigned i = 0 ; i<p->size ; i++)
    {
      unsigned sidx = reinterpret_cast<unsigned (**)(Node<T> *,
                                                     ...
                                                     unsigned)>
                      (func)[p->sons[i]->header.typeNode](
                          p->sons[i], func, out, idx, globalStamp);
