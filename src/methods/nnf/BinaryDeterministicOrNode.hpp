...
template <class T, typename U> class BinaryDeterministicOrNode : public Node<T>
{
 public:
  ...
  void init(DataBranch<T> &left, DataBranch<T> &right)
  {
    ...
    unsigned pos = 0;
    for(auto &l : left.unitLits) data[pos++] = l.intern();
    for(auto &v : left.freeVars) data[pos++] = v;
    for(auto &l : right.unitLits) data[pos++] = l.intern();
    for(auto &v : right.freeVars) data[pos++] = v;
  } // init
