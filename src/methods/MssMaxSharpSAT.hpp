...
template <class T>
class MssMaxSharpSAT : public MethodManager {
  enum TypeDecision { NO_DEC, EXIST_DEC, MAX_DEC };

  struct MssMaxSharpSatResult {
    T count;
    u_int8_t *valuation;

    MssMaxSharpSatResult() : count(T(0)), valuation(NULL) {}
    MssMaxSharpSatResult(const T c, u_int8_t *v) : count(c), valuation(v) {}
 ...
 private:
  ...
  CacheManager<MssMaxSharpSatResult> *m_cacheMax;
  ...
  MssMaxSharpSatResult m_scale = {T(1), NULL};
  MssMaxSharpSatResult m_maxCount = {T(0), NULL};
 ...
 public:
   ...
   */
  MssMaxSharpSAT(po::variables_map &vm, std::string &meth, bool isFloat,
    ...
    m_cacheInd = CacheManager<T>::makeCacheManager(vm, m_problem->getNbVar(),
                                                   m_specs, m_out);
    m_cacheMax = CacheManager<MssMaxSharpSatResult>::makeCacheManager(
 ...
 private:
   ...
   */
  void printSolution(MssMaxSharpSatResult &solution, char status) {
  ...
  */
   ...
   * compute result.
   */
  void updateBound(MssMaxSharpSatResult &result, std::vector<Var> &vars) {
  ...
  void searchMaxValuation(std::vector<Var> &setOfVar,
    ...
    if (nbComponent) {
      ...
      for (int cp = 0; cp < nbComponent; cp++) {
        ...
        TmpEntry<MssMaxSharpSatResult> cb =
            m_cacheMax->searchInCache(connected);
        ...
        if (cb.defined) {
          ...
          MssMaxSharpSatResult tmpResult;
          searchMssMaxSharpSatDecision(connected, out, tmpResult);
   ...
   */
  void searchMssMaxSharpSatDecision(std::vector<Var> &connected,
                                    std::ostream &out,
                                    MssMaxSharpSatResult &result) {
    if (m_stopProcess) return;
  ...
  void compute(std::vector<Var> &setOfVar, std::ostream &out,
               MssMaxSharpSatResult &result, bool warmStart = true) {
