...
class SemiringOperation : public Operation<U, T> {
 private:
  ProblemManager *m_problem;

  const T top = T::one();
  const T bottom = T::zero();

 public:
  SemiringOperation() = delete;
  ...
  /**
     ...
     weights.
   */
  SemiringOperation(ProblemManager *problem)
  ...
  */
  T manageDeterministOr(DataBranch<T> *elts, unsigned size) {
    ...
    for (unsigned i = 0; i < size; i++) {
      ...
      T tmp = T::one();
      for (auto &l : elts[i].unitLits) {
	tmp *= T(m_problem->getWeightLit<T>(l));
      }
      for (auto &v : elts[i].freeVars) {
	tmp *= T(m_problem->getWeightLit<T>(Lit::makeLitTrue(v)))+T(m_problem->getWeightLit<T>(Lit::makeLitFalse(v))); // Force semiring operations 
      }

      ret += elts[i].d * tmp; // computeWeightUnitFree<T>(elts[i].unitLits, elts[i].freeVars);
    }
#if 0
    if (ret > maxCount) {
      maxCount = ret;
  ...
  /**
     ...
     \return T::one() as number of models.
   */
  inline T manageTop(std::vector<Var> &component) { return top; }

  /**
     Return true, that is given by the value 1 (of the semiring).
  ...
  T manageBranch(DataBranch<T> &e) {
    T tmp = T::one();
      for (auto &l : e.unitLits) {
	tmp *= T(m_problem->getWeightLit<T>(l));
      }
      for (auto &v : e.freeVars) {
	tmp *= T(m_problem->getWeightLit<T>(Lit::makeLitTrue(v)))+T(m_problem->getWeightLit<T>(Lit::makeLitFalse(v))); // Force semiring operations as m_weightVar is not parameterized by T yet. 
      }

      return e.d * tmp;// m_problem->computeWeightUnitFree<T>(e.unitLits, e.freeVars);
  }  // manageBranch
 ...
 */
    U count(T &result) { return U(result); }  // count

  /**
     Cannot be called, then throws an exception!
   */
  U count(T &result, std::vector<Lit> &assum) {
