 ...
 */
void WrapperGlucose::getSimplifiedFormula(ProblemManager &pout) {
  try {
    ...
    for (int i = 0; i < s.trail.size(); i++) {
      std::vector<Lit> cl;
      cl.push_back(
          Lit::makeLit(Glucose::var(s.trail[i]), Glucose::sign(s.trail[i])));
      ret.push_back(cl);
    }
    ...
    for (int i = 0; i < s.clauses.size(); i++) {
      ...
      for (int j = 0; j < c.size() && !isSAT; j++) {
        if (s.isUndef(c[j]))
          cl.push_back(Lit::makeLit(Glucose::var(c[j]), Glucose::sign(c[j])));
        isSAT = s.isSAT(c[j]);
      }
