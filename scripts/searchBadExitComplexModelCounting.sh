...
is_executable_ready
export TESTED_METHOD="${TESTED_METHOD:-../c++/counter/build/counter --branching-heuristic classic -i}"
fuzz_loop "gen_complex_cnf 200" "$(dirname "$0")/crash_check.sh" "${1:-2}" "${2:-10}"
