...
generateSatisfiableCNF()
{
    ...
    do
        ...
        if [ $rdm -lt 10 ]
        then
            echo "c p weight $i 0.0$rdm 0" >> /tmp/test.cnf
        else
            echo "c p weight $i 0.$rdm 0" >> /tmp/test.cnf
        fi

        rest=$((100 - rdm))
        if [ $rest -lt 10 ]; then rest="0$rest"; fi

        echo "c p weight -$i 0.$rest 0" >> /tmp/test.cnf
    done
...
do
    ...
    timeout $TIMEOUT $1 $benchName /tmp/queries.txt    
    code=$?
