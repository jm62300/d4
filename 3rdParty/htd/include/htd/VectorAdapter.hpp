 ...
 *
 *  @note This adapter is necessary when it is desired to return a vector hidden
 * behind an instance of htd::Collection and htd::ConstCollection from a
 * function because otherwise the iterators used inside the collection wrappers
 * are invalidated.
 */
...
class VectorAdapter {
 public:
   ...
   */
  VectorAdapter(void)
      : container_(std::make_shared<std::vector<T, Allocator>>()) {}

  /**
   *  Constructor for an adapter of a vector.
   *
   *  @param[in] collection   The vector which shall be copied to the new
   * adapter.
   */
  VectorAdapter(const std::vector<T>& collection)
      : container_(std::make_shared<std::vector<T, Allocator>>(collection)) {}

  /**
   *  Constructor for an adapter of a vector.
   *
   *  @param[in] collection   The vector which shall be transferred to the new
   * adapter.
   */
  VectorAdapter(std::vector<T>&& collection)
      : container_(std::make_shared<std::vector<T, Allocator>>()) {
   ...
   * adapter.
   */
  VectorAdapter(const htd::ConstCollection<T>& collection)
      : container_(std::make_shared<std::vector<T, Allocator>>(
   ...
   */
  VectorAdapter(const VectorAdapter<T>& original)
   ...
   *  @param[in] original  The original vector adapter.
   */
  VectorAdapter(VectorAdapter<T>&& original)
   ...
   */
  std::size_t size(void) const { return container_->size(); }
   ...
   */
  std::vector<T, Allocator>& container(void) { return *container_; }
   ...
   *  @return The underlying vector wrapped by the adapter.
   */
  const std::vector<T, Allocator>& container(void) const { return *container_; }
  ...
  htd::Iterator<T> begin(void) {
    return htd::VectorAdapterIteratorWrapper<
        typename std::vector<T, Allocator>::iterator, T, Allocator>(
        container_, container_->begin());
  }
  ...
  htd::ConstIterator<T> begin(void) const {
    return htd::ConstIterator<T>(static_cast<htd::ConstIteratorBase<T>*>(
        new htd::VectorAdapterConstIteratorWrapper<
            typename std::vector<T, Allocator>::const_iterator, T, Allocator>(
            container_, container_->begin())));
  }
  ...
  htd::Iterator<T> end(void) {
    return htd::VectorAdapterIteratorWrapper<
        typename std::vector<T, Allocator>::iterator, T, Allocator>(
        container_, container_->end());
  }

  /**
   *  Getter for a const_iterator pointing to the end of the elements in the
   * vector.
   *
   *  @return A const_iterator pointing to the end of the elements in the
   * vector.
   */
   ...
   */
  T& operator[](htd::index_t index) { return container_->at(index); }
   ...
   *  @param[in] original  The original vector.
   */
  VectorAdapter<T, Allocator>& operator=(
      const VectorAdapter<T, Allocator>& original) {
    std::swap(original, *container_);
   ...
   */
  VectorAdapter<T, Allocator>& operator=(
      VectorAdapter<T, Allocator>&& original) {
    container_ = std::move(original.container_);
   ...
   */
  VectorAdapter<T, Allocator>& operator=(
      const htd::ConstCollection<T>& original) {
    container_->clear();
   ...
   *
   *  @return True if the collection of elements in the wrapped vector is equal
   * to the collection at the right-hand side of the operator, false otherwise.
   */
  ...
  inline bool operator==(const htd::Collection<T>& rhs) const {
    return size() == rhs.size() &&
           htd::equal(begin(), end(), rhs.begin(), rhs.end());
  }
   ...
   *  @param[in] rhs  The collection at the right-hand side of the operator.
   *
   *  @return True if the collection of elements in the wrapped vector is not
   * equal to the collection at the right-hand side of the operator, false
   *          otherwise.
   */
  inline bool operator!=(const VectorAdapter<T, Allocator>& rhs) const {
    return *container_ != *(rhs.container_);
