...
#ifdef _WIN32
#include <intrin.h>
#define CADICAL_PREFETCH(addr) _mm_prefetch((const char*)(addr), _MM_HINT_T0)
#define CADICAL_UNREACHABLE() __assume(0)
#define CADICAL_PRETTY_FUNCTION __FUNCSIG__
static inline unsigned cadical_clz (unsigned x) {
  unsigned long index;
  if (_BitScanReverse (&index, x))
    return 31 - index;
  return 32;
}
#define CADICAL_CLZ(x) cadical_clz (x)
#else
#define CADICAL_PREFETCH(addr) __builtin_prefetch(addr)
#define CADICAL_UNREACHABLE() __builtin_unreachable()
#define CADICAL_PRETTY_FUNCTION __PRETTY_FUNCTION__
#define CADICAL_CLZ(x) __builtin_clz (x)
#endif
