...
HyperedgeSet SetCover::NodeCover1(const VertexSet &Vertices,
                                  const HyperedgeSet &HEdges, bool bDeterm) {
  ...
  for (auto v : Vertices) {
    /* for (auto it = MyH->getVertexNeighborsBegin(v); it !=
       MyH->getVertexNeighborsEnd(v); it++)
            (*it)->setLabel(-1); */
    v->setLabel(0);
  }

  // Unhide edges that can be used to cover the vertices
  for (auto he : HEdges) he->setLabel(0);

  // STEP 1: Choose all hyperedges that cover a node which is not contained in
  // any other hyperedge
  for (auto v : Vertices) {
  ...
  // STEP 2: Choose iteratively hyperedges that cover a maximum number of
  // uncovered nodes
  while (iNbrOfUncovNodes > 0) {
    ...
    for (auto he : HEdges) {
      if (he->getLabel() > iMax) {
        ...
        iMax = he->getLabel();
      } else if (he->getLabel() == iMax)
        CovCand.push_back(he);

      // Reset hyperedge labels
      if (he->getLabel() > 0) he->setLabel(0);
    }
    ...
    if (bDeterm) {
      hedge = CovCand[0];
      for (auto he : CovCand)
        if (he->getId() < hedge->getId()) hedge = he;
    } else
...
HyperedgeSet SetCover::NodeCover2(const VertexSet &Vertices,
                                  const HyperedgeSet &HEdges, bool bDeterm) {
  size_t iNbrOfCovEdges, iNbrOfUncovNodes, iContained;
  int i;
  HyperedgeSet CovEdges{};
  double iMax;
  std::unordered_map<VertexSharedPtr, double> NodeWeights;
  std::unordered_map<HyperedgeSharedPtr, double> EdgeWeights;
  HyperedgeSharedPtr hedge;
  HyperedgeVector CovCand;
  ...
  */
  ...
  for (auto v : Vertices) {
    ...
    for (auto he : MyH->allVertexNeighbors(v)) {
      he->setLabel(-1);
      if (HEdges.find(he) != HEdges.end()) ++iContained;
    }
    v->setLabel(0);
  ...
  for (auto v : Vertices)
    if (NodeWeights[v] > 0)
      for (auto he : MyH->allVertexNeighbors(v))
        if (he->getLabel() >= 0) EdgeWeights[he] += NodeWeights[v];
...
HyperedgeSet SetCover::cover(const VertexSet &Vertices,
                             const HyperedgeSet &HEdges) {
