...
class Subedges {
 private:
  ...
  int MyK;
  std::unordered_map<HyperedgeSharedPtr, HyperedgeVector, NamedEntityHash>
      MySubedges;
 ...
 public:
  ...
  void push_back(const HyperedgeSharedPtr &orig,
                 const HyperedgeSharedPtr &subedge);

  auto size() const { return MySubedges.size(); }

  size_t size(const HyperedgeSharedPtr &orig) const;

  auto allSubedges(const HyperedgeSharedPtr &orig)
      -> decltype(make_iterable(MySubedges.at(orig).begin(),
                                MySubedges.at(orig).end())) {
    return make_iterable(MySubedges.at(orig).begin(),
                         MySubedges.at(orig).end());
  }

  auto allSubedges(const HyperedgeSharedPtr &orig) const
      -> decltype(make_iterable(MySubedges.at(orig).begin(),
                                MySubedges.at(orig).end())) {
  ...
  auto &subedges(const HyperedgeSharedPtr &orig) const {
    return MySubedges.at(orig);
  }
};
}
