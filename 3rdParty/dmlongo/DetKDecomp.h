...
class DetKDecomp : public Decomp {
 protected:
  ...
  // Divides a set of hyperedges into inner hyperedges and those containing
  // given vertices
  size_t divideCompEdges(const HyperedgeVector &HEdges,
                         const VertexSet &Vertices, HyperedgeVector &Inner,
                         HyperedgeVector &Bound) const;

  // Returns the partitions to a given separator that are known to be
  // decomposable or undecomposable
  ...
  bool covers(const SeparatorSharedPtr &Edges,
              const VertexSet &Vertices) const {
    return covers(Edges->edges(), Vertices);
  }

  // Builds a hypertree decomposition according to k-decomp by covering
  // connector nodes
  virtual HypertreeSharedPtr decomp(const HyperedgeVector &HEdges,
                                    const VertexSet &Connector = VertexSet(),
                                    int RecLevel = 0) const;
  virtual HypertreeSharedPtr decomp(const DecompComponent &comp,
                                    int recLevel) const {
