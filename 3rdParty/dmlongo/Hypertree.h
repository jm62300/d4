...
class Hypertree : public std::enable_shared_from_this<Hypertree> {
 ...
 public:
  ...
  void insChild(const std::shared_ptr<Hypertree> &Child,
  ...
  void remChildren(bool SetParent = true,
                   const std::shared_ptr<Hypertree> &NewParent = nullptr);
  ...
  // setFec(unique_ptr<FractionalEdgeCover> &fec) {
  //	MyFec = move(fec);
  //}
  ...
  auto allLambda() const
      -> decltype(make_iterable(MyLambda.begin(), MyLambda.end())) {
    return make_iterable(MyLambda.begin(), MyLambda.end());
  }

  // Returns all children
  auto allChildren() const
      -> decltype(make_iterable(MyChildren.begin(), MyChildren.end())) {
  ...
  // Removes redundant nodes (default compares Chi-Labels, else compares
  // Lambda-Labels)
  // TODO: Lambda! Shrinking has to be done differently with fractional edge
  // covers 2018-07-30: Chi Shrinking done, has to be tested
