...
class DecompComponent {
 ...
 public:
  ...
  bool contains(const HyperedgeSharedPtr &edge) const {
    return find(MyComp.begin(), MyComp.end(), edge) != MyComp.end();
  }
  HyperedgeSharedPtr containsOneOf(
  ...
  HyperedgeSharedPtr first() const {
    if (MyComp.size() > 0)
      return *(MyComp.begin());
    else
      return HyperedgeSharedPtr(nullptr);
  }
  ...
  const HyperedgeSharedPtr &operator[](std::size_t idx) const {
    return MyComp[idx];
  }
};
