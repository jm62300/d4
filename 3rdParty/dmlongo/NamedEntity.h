...
class NamedEntity {
 ...
 public:
  NamedEntity(uint id = 0, const std::string& name = "") : Id{id}, Name{name} {
    Label = 0;
  }
...
template <typename T>
bool isSubset(const std::unordered_set<T, NamedEntityHash>& a,
              const std::unordered_set<T, NamedEntityHash>& b) {
