...
class Hypergraph : NamedEntity {
 private:
  ...
  // another graph The desctructor then keeps edges and vertices
  HyperedgeSet Edges;
  VertexSet Vertices;
 ...
 public:
  ...
  // void buildHypergraph(PACEParser &P, int n, int m);
  ...
  bool hasEdge(const HyperedgeSharedPtr &he) const {
    return Edges.find(he) != Edges.end();
  }
  ...
  auto allVertices()
      -> decltype(make_iterable(Vertices.begin(), Vertices.end())) {
  ...
  size_t nbrOfVertexNeighbors(const VertexSharedPtr &v) const {
    return VertexNeighbors.at(v).size();
  }

  auto allVertexNeighbors(const VertexSharedPtr &v)
      -> decltype(make_iterable(VertexNeighbors[v].begin(),
                                VertexNeighbors[v].end())) {
    return make_iterable(VertexNeighbors[v].begin(), VertexNeighbors[v].end());
  }

  auto allVertexNeighbors(const VertexSharedPtr &v) const
      -> decltype(make_iterable(VertexNeighbors.at(v).begin(),
                                VertexNeighbors.at(v).end())) {
    return make_iterable(VertexNeighbors.at(v).begin(),
                         VertexNeighbors.at(v).end());
  }
  ...
  */
  ...
  size_t nbrOfEdgeNeighbors(HyperedgeSharedPtr &e) const {
    return EdgeNeighbors.at(e).size();
  }

  auto allEdgeNeighbors(const HyperedgeSharedPtr &e)
      -> decltype(make_iterable(EdgeNeighbors[e].begin(),
                                EdgeNeighbors[e].end())) {
    return make_iterable(EdgeNeighbors[e].begin(), EdgeNeighbors[e].end());
  }

  auto allEdgeNeighbors(const HyperedgeSharedPtr &e) const
      -> decltype(make_iterable(EdgeNeighbors.at(e).begin(),
                                EdgeNeighbors.at(e).end())) {
    return make_iterable(EdgeNeighbors.at(e).begin(),
                         EdgeNeighbors.at(e).end());
  }
