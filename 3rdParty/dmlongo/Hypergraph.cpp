...
void Hypergraph::labelReachEdges(const HyperedgeSharedPtr &edge,
  ...
  for (auto e : EdgeNeighbors.at(edge))
    if (e->getLabel() == 0) labelReachEdges(e, label);
}
...
/*
void Hypergraph::buildHypergraph(PACEParser &P, int n, int m) {
        VertexVector vertices;
        HyperedgeSharedPtr edge;
...
void Hypergraph::setVertexLabels(int value) const {
  for (auto v : Vertices) v->setLabel(value);
}

void Hypergraph::setEdgeLabels(int value) const {
  for (auto he : Edges) he->setLabel(value);
}
...
void Hypergraph::insertEdge(const HyperedgeSharedPtr &edge) {
  ...
  for (auto v : edge->allVertices()) {
    if (find(Vertices.begin(), Vertices.end(), v) == Vertices.end())
      Vertices.insert(v);
    EdgeNeighbors[edge].insert(VertexNeighbors[v].cbegin(),
                               VertexNeighbors[v].cend());
    for (auto e : VertexNeighbors[v]) EdgeNeighbors[e].insert(edge);
...
bool Hypergraph::isConnected() const {
  ...
  if (getNbrOfEdges() > 0) labelReachEdges(*Edges.begin());

  for (auto e : Edges)
    if (e->getLabel() == 0) return false;
...
HyperedgeVector Hypergraph::getMCSOrder() {
  ...
  do {
    ...
    if (he_iter != Edges.end()) {
      ...
      he = *he_iter;
      for (auto n : allEdgeNeighbors(he)) max_card += n->getLabel();
      candidates.push_back(he);

      // Search for the node with highest connectivity,
      // i.e., with highest number of neighbours in the set of nodes already
      // removed
      for (; he_iter != Edges.end(); he_iter++) {
        he = *he_iter;
        if (he->getLabel() == 0) {
          tmp_card = 0;
          for (auto n : allEdgeNeighbors(he)) tmp_card += n->getLabel();
  ...
  if (k <= Edges.size()) {
    ...
    while ((indices = comb.next()) != nullptr) {
      ...
      for (auto v : edges[indices[0]]->allVertices()) {
        ...
        for (int i = 1; indices[i] != -1 && found; i++)
          if (find(edges[indices[i]]->allVertices().begin(),
                   edges[indices[i]]->allVertices().end(),
                   v) == edges[indices[i]]->allVertices().end())
            found = false;

        if (found) vertices.push_back(v);
      }

      if (vertices.size() > maxBip) maxBip = vertices.size();
    }
  } else {
    VertexVector vertices;
    ...
    for (auto v : edges[0]->allVertices()) {
      ...
      for (int i = 1; i < edges.size() && found; i++)
        if (find(edges[i]->allVertices().begin(), edges[i]->allVertices().end(),
                 v) == edges[i]->allVertices().end())
          found = false;
...
int Hypergraph::vcDimension() const {
  ...
  for (i = 1; i <= maxVC; i++) {
    ...
    // For each subset X of size vc check if it is shattered, if X is shattered
    // then vc is at least i
    int *indices;
    ...
    while ((indices = cit.next()) != nullptr && !shattered) {
      bool check_x = true;
      set_type set_x;
      for (int j = 0; indices[j] != -1; j++) set_x.insert(vertices[indices[j]]);
      ...
      // if there is a subset such that this check fails (checkX = false), then
      // X is not shattered.
      for (powerset_type::iterator iter = pow_set_x.begin();
           iter != pow_set_x.end() && check_x; ++iter) {
        bool edge_found = false;
        for (auto e_it = Edges.begin(); e_it != Edges.end() && !edge_found;
             e_it++) {
          set_type help_x;

          for (auto v : set_x) {
            if (find((*e_it)->allVertices().begin(),
                     (*e_it)->allVertices().end(),
                     v) != (*e_it)->allVertices().end())
              help_x.insert(v);
          }

          if (help_x == *iter) edge_found = true;
        }

        if (!edge_found) check_x = false;
      }

      if (check_x) shattered = true;
    }

    if (!shattered) return i - 1;
  }
...
int Hypergraph::arity() const {
  ...
  int help = 0;
  for (auto e : Edges)
    if ((help = e->getNbrOfVertices()) > ari) ari = help;
