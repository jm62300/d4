...
int* CombinationIterator::next() {
  ...
  if (k <= n) {
    if (indices[0] == -1) {
      ...
      i++;
      if (found)
        for (++i; i < stage; ++i) indices[i] = indices[i - 1] + 1;
      else {
        stage++;
        if (stage <= k) {
          found = true;
...
void CombinationIterator::reset() {
  for (auto i = 0; i <= k; i++) indices[i] = -1;
  this->stage = 1;
}

void CombinationIterator::setStage(int stage) { this->stage = stage; }
