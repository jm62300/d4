...
class Hyperedge : public NamedEntity {
 ...
 public:
  // Helper Constructors
  Hyperedge(const std::string &name);
  Hyperedge(const std::string &name, const VertexSet &vertices);
  Hyperedge(int id, const std::string &name)
      : Hyperedge(id, name, VertexSet()) {
  }  // Hyperedge(int id, const string& name, const VertexSet::const_iterator
  // &begin, const VertexSet::const_iterator &end) : Hyperedge(id,name,
  // VertexSet(begin, end)) { }

  // Real Constructor
  Hyperedge(int id, const std::string &name, const VertexSet &vertices)
      : NamedEntity(id, name), Vertices{vertices} {};
  virtual ~Hyperedge();
  ...
  VertexSharedPtr find(const std::string &name);

  bool sameVertices(const Hyperedge &e) const {
  ...
  vector<Vertex *>::const_iterator getVerticesCBegin() const { return
  MyVertices.cbegin(); } vector<Vertex *>::const_iterator getVerticesCEnd()
  const { return MyVertices.cend(); }
  */
