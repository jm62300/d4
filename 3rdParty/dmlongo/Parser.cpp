...
Parser::~Parser() {
  ...
  std::vector<std::set<int> *>::iterator sVectorIter;
  std::vector<bool>::iterator bVectorIter;

  for (sVectorIter = MyAtomVars.begin(); sVectorIter != MyAtomVars.end();
       sVectorIter++)
    delete *sVectorIter;

  for (sVectorIter = MyAtomNeighbours.begin();
       sVectorIter != MyAtomNeighbours.end(); sVectorIter++)
    delete *sVectorIter;

  for (sVectorIter = MyVarAtoms.begin(); sVectorIter != MyVarAtoms.end();
       sVectorIter++)
    delete *sVectorIter;

  for (sVectorIter = MyVarNeighbours.begin();
       sVectorIter != MyVarNeighbours.end(); sVectorIter++)
    delete *sVectorIter;

  for (bVectorIter = MyDefAtomUsed.begin(); bVectorIter != MyDefAtomUsed.end();
       bVectorIter++, iDefAtom++)
    if (!*bVectorIter) delete MyDefAtoms[iDefAtom];

  for (bVectorIter = MyDefVarUsed.begin(); bVectorIter != MyDefVarUsed.end();
       bVectorIter++, iDefVar++)
    if (!*bVectorIter) delete MyDefVariables[iDefVar];
}
...
void Parser::SyntaxError(const char *cMsg, bool bLineNbr) {
  if (bLineNbr)
    std::cerr << "Error in Ln " << iMyLineNumber << ", Col " << iMyColumnNumber
              << ": " << cMsg << std::endl;
  else
    std::cerr << "Error: " << cMsg << std::endl;
  exit(EXIT_FAILURE);
}

void Parser::SyntaxWarning(const char *cMsg, bool bLineNbr) {
  if (bLineNbr)
    std::cerr << "Warning in Ln " << iMyLineNumber << ", Col "
              << iMyColumnNumber << ": " << cMsg << std::endl;
  else
    std::cerr << "Warning: " << cMsg << std::endl;
  ++iMyNbrOfWarnings;
}
...
void Parser::fillBuffer() {
  if (!MyFile.eof()) {
    ...
    for (int i = 0; i < IF_BUFFER_SIZE; i++) cMyBuffer[i] = '\0';
    ...
    if (iMyEndOfAtoms == 0) SyntaxError("Unexpected end of file.", true);
  }
}
  ...
  do {
    switch (cMyBuffer[iMyBufferPos]) {
      ...
      case 9:  // Ignore tabulators
        ++iMyBufferPos;
        ++iMyColumnNumber;
        while (iMyColumnNumber % 8 != 1) ++iMyColumnNumber;
        break;
      case '\n':  // Ignore newlines
        ++iMyBufferPos;
        ++iMyLineNumber;
      ...
      case '%':  // Ignore comments
        do {
          ++iMyBufferPos;
          ++iMyColumnNumber;
          if (cMyBuffer[iMyBufferPos] ==
              '\0')  // End of buffer reached -> refill
            fillBuffer();
...
char *Parser::readIdentifier() {
  ...
  do {
    if (cMyBuffer[iMyBufferPos] == '\0')  // End of buffer reached -> refill
      ...
      if (((cMyBuffer[iMyBufferPos] >= 'A') &&
           (cMyBuffer[iMyBufferPos] <= 'Z')) ||
          ((cMyBuffer[iMyBufferPos] >= 'a') &&
           (cMyBuffer[iMyBufferPos] <= 'z')) ||
          ((cMyBuffer[iMyBufferPos] >= '0') &&
           (cMyBuffer[iMyBufferPos] <= '9')) ||
          (cMyBuffer[iMyBufferPos] == '_') ||
          (cMyBuffer[iMyBufferPos] == ':')) {
  ...
  if (iIdentifierSize == 0) SyntaxError("Illegal or missing identifier.", true);

  // Allocate character array for identifier and copy identifier from the
  // identifier buffer in this array
...
bool Parser::readIdentifier(const char *cIdent) {
  ...
  while (cIdent[i] != '\0') {
    if (cMyBuffer[iMyBufferPos] == '\0')  // End of buffer reached -> refill
      fillBuffer();
    if (cMyBuffer[iMyBufferPos] != cIdent[i++]) return false;
    ++iMyBufferPos;
    ++iMyColumnNumber;
  }
  ...
  do {
    if (cMyBuffer[iMyBufferPos] == '\0')  // End of buffer reached -> refill
      ...
      if ((cMyBuffer[iMyBufferPos] >= '0') &&
        ...
        if (iNbrOfDigits == 4) bEndArity = true;
      } else
        bEndArity = true;
    }
  } while (!bEndArity);
  // Until a non-numerical character was found or the maximum number of digits
  // was exceeded

  if (iArity == 0) SyntaxError("Illegal arity number.", true);
...
void Parser::readVarDefs() {
  ...
  std::vector<char *>::iterator VectorIter;
  ...
  do {
    ...
    while ((VectorIter != MyDefVariables.end()) &&
           (strcmp(*VectorIter, cIdentifier) != 0))
      ++VectorIter;
...
void Parser::readAtomDefs() {
  ...
  do {
    ...
    while ((VectorIter != MyDefAtoms.end()) &&
           (strcmp(*VectorIter, cIdentifier) != 0))
      ++VectorIter;
    ...
    readIgnoreText();
    if (!readIdentifier("/")) SyntaxError("Illegal character.", true);
...
bool Parser::readOpenDef() { return readIdentifier("<"); }
...
bool Parser::readSeparator() { return readIdentifier(","); }
  ...
  while (readOpenDef()) {
    ...
    if (readIdentifier("def")) {
      if (readIdentifier("Var"))  // Read "defVar" identifier
        bVarDef = true;
      else if (readIdentifier("Rel"))  // Read "defRel" identifier
        bAtomDef = true;
      else
...
void Parser::readAtom() {
  ...
  Set = new std::set<int>;
  if (Set == NULL) writeErrorMsg("Error assigning memory.", "Parser::readAtom");
...
void Parser::readVariable() {
  ...
  std::set<int> *Set = NULL;
  std::pair<std::set<int>::iterator, bool> SetInsertRet;
  ...
  while ((VectorIter != MyVariables.end()) &&
         (strcmp(*VectorIter, cIdentifier) != 0)) {
    ++VectorIter;
    ++iVar;
  }
  ...
  // Write syntax warning if the variable occurs multiple times in the actual
  // atom
  ...
  // Update the neighbourhood relation between the actual atom and the atoms
  // already stored before
  for (SetIter = MyVarAtoms[iVar]->begin(); SetIter != MyVarAtoms[iVar]->end();
       SetIter++)
    if (*SetIter != iAtom) {
  ...
  // Update the neighbourhood relation between the actual variable and the
  // variables already stored before
  for (SetIter = MyAtomVars[iAtom]->begin();
       SetIter != MyAtomVars[iAtom]->end(); SetIter++)
    if (*SetIter != iVar) {
...
void Parser::parseFile(const char *cNameOfFile) {
  ...
  std::vector<bool>::iterator bVectorIter;
  std::vector<char *>::iterator cVectorIter;

  // Open input file stream
  MyFile.open(cNameOfFile, std::ios::in);

  // Check if file opening was successful
  if (!MyFile.is_open()) SyntaxError("Error opening file.");
  ...
  if (!bMyDef) {
    for (cVectorIter = MyDefAtoms.begin(); cVectorIter != MyDefAtoms.end();
         cVectorIter++)
      delete *cVectorIter;
    for (cVectorIter = MyDefVariables.begin();
         cVectorIter != MyDefVariables.end(); cVectorIter++)
      delete *cVectorIter;
    MyDefAtoms.clear();
  ...
  // Write syntax warning if EOF does not appear during reading the ignore text,
  // i.e., there appears some text after the end-token was found
  if (!bMyEOF) {
    strcpy(cErrorMsg,
           "Illegal character after termination symbol \".\" in line ");
  ...
  if (bMyDef) {
    ...
    for (bVectorIter = MyDefAtomUsed.begin();
         bVectorIter != MyDefAtomUsed.end(); bVectorIter++, iDefAtom++)
      if (!*bVectorIter) {
        strcpy(cErrorMsg, "Atom \"");
...
int Parser::getNbrOfAtoms() const { return (int)MyAtoms.size(); }

int Parser::getNbrOfVars() const { return (int)MyVariables.size(); }
...
int Parser::getNbrOfWarnings() const { return iMyNbrOfWarnings; }
...
int Parser::getNextAtomVar(bool bReset) {
  static std::vector<std::set<int> *>::iterator VectorIter;
  static std::set<int>::iterator SetIter;
