...
/*
***Description***
The method selects an initial subset within a set of hyperedges such that a
given set of nodes is covered.
...
OUTPUT: Set: Selection of hyperedges in Edges such that all nodes in Nodes are
covered CovWeights: Array with the number of connector nodes covered by each
hyperedge return: Number of selected hyperedges; -1 if nodes cannot be covered
*/
...
/*
***Description***
The method selects the next subset within a set of hyperedges such that a given
set of nodes is covered.

INPUT:	Nodes: Array of nodes to be covered
                Edges: Array of hyperedges
                Set: Integer array of indices in Edges representing a subset
selection bInComp: Boolean array indicating the position of each hyperedge
                CovWeights: Array with the number of connector nodes covered by
each hyperedge iSize: Number of elements in Edges, bInComp, and CovWeights
OUTPUT: Set: Selection of hyperedges in Edges such that all nodes in Nodes are
covered return: Number of selected hyperedges; -1 if there is no alternative
selection
*/
...
int DetKDecomp::coverNodes(HyperedgeVector &Edges, std::vector<int> &Set,
  ...
  if (Reconstr) {
    ...
    for (nbr_sel = 0; Set[nbr_sel + 1] != -1; nbr_sel++) {
      pos = Set[nbr_sel];
      if (InComp[pos]) ++in_comp_sel;

      tmp_labels.clear();
      tmp_labels.resize(Edges[pos]->getNbrOfVertices());
      // if(iTmpLabels == NULL)
      //	writeErrorMsg("Error assigning memory.",
      //"DetKDecomp::coverNodes");
  ...
  while (!tle && !covered) {
    for (back = false; !covered; pos++) {
      ...
      if (select) {
        ...
        // Save labels of nodes in the actual hyperedge and label these nodes by
        // 1
        i = 0;
        for (auto v : Edges[pos]->allVertices()) {
          tmp_labels[i] = v->getLabel();
          v->setLabel(1);
          if (tmp_labels[i] == 0) --Uncovered;
          i++;
        }
        label_stack.push_back(tmp_labels);

        // Check whether all nodes are covered
        if (Uncovered == 0) covered = true;
      }
    }
...
/*
***Description***
The method divides a given set of hyperedges into inner hyperedges and its
boundary according to a given set of boundary nodes. The inner hyperedges are
those not containing a boundary node. All other hyperedges containing a boundary
node (within the given set or outside) belong to the boundary hyperedges.
...
OUTPUT: Inner: Inner hyperedges not containing a boundary node
                ...
                return: Index in Bound that separates boundary hyperedges within
and outside the given set of hyperedges; all hyperedges before this index are
inside the given set and all hyperedges starting at this index are outside the
given set.
*/

size_t DetKDecomp::divideCompEdges(const HyperedgeVector &HEdges,
                                   const VertexSet &Vertices,
                                   HyperedgeVector &Inner,
  ...
  for (auto he : HEdges)
    if (edgeLabels[he] > 0) Inner.push_back(he);

  edgeLabels.clear();
  for (auto he : outerb) edgeLabels[he] = 1;

  // Remove redundant hyperedges from OuterB; such a hyperedge is redundant if
  // the set of its boundary nodes is covered by some other hyperedge in OuterB
  for (auto it = outerb.begin(); it != outerb.end(); it++) {
    // Label all boundary nodes by 1
    for (auto v : Vertices) vertexLabels[v] = (1);

    // Reset the labels of all nodes of the actual hyperedge in OuterB
    for (auto v : (*it)->allVertices()) vertexLabels[v] = 0;

    // Check whether some hyperedge in OuterB in the neighbourhood of the actual
    // hyperedge contains no node labeled with 1; in this case it can be removed
    // from OuterB since its boundery nodes are covered by the actual hyperedge
    // in OuterB
    for (auto he : MyHg->allEdgeNeighbors(*it))
      if (edgeLabels[he] != 0) {
        ...
        if (covered) outerb.remove(he);
      }
  }

  // Store hyperedges containing a boundary node in the array
  for (auto he : innerb) Bound.push_back(he);
  for (auto he : outerb) Bound.push_back(he);
...
/*
***Description***
The method checks whether the actual separator occurs in the MySeps list of
already used separators. If so, the method returns two lists of hyperedges
identifiying the decomposable and undecomposable partitions. It is assumed that
separator hyperedges are labeled by -1 and no other hyperedges are labeled by
-1.
...
*/
/*
bool DetKDecomp::getSepParts(int SepSize, HE_VEC **Separator, list<Hyperedge *>
**SuccParts, list<Hyperedge *> **FailParts)
{
        int i;
        bool found = false;
        list<list<Hyperedge *> *>::iterator SuccPartIter, FailPartIter;

        // Check whether the actual separator is known to be decomposable or
undecomposable SuccPartIter = MySuccSepParts.begin(); FailPartIter =
MyFailSepParts.begin(); for(auto sep : MySeps) { found = true; i = 0; for (auto
he : *sep) { if (he->getLabel() != -1) { found = false; break;
                        }
                        i++;
                }
...
INPUT:	HEdges: Pointer to a list of hyperedges
                iLabel: Label
OUTPUT: return: true if there exists an edge in HEdges labeled by iLabel;
otherwise false
*/
...
bool DetKDecomp::containsLabel(std::list<Hyperedge *> *HEdges, int iLabel) {
  ...
  for (ListIter = HEdges->begin(); ListIter != HEdges->end(); ListIter++)
    if ((*ListIter)->getLabel() == iLabel) return true;
...
/*
***Description***
The method decomposes the hyperedges in a subhypergraph as described in Gottlob
and Samer: A Backtracking-Based Algorithm for Computing
...
*/
bool DetKDecomp::covers(const HyperedgeVector &Edges,
...
HypertreeSharedPtr DetKDecomp::decomp(const HyperedgeVector &HEdges,
                                      const VertexSet &Connector,
                                      int RecLevel) const {
  int i, j;
  int z = 0;
  int thr;
  ...
  */
  ...
  if ((htree = decompTrivial(HEdges, Connector)) != nullptr) return htree;

  if (tle) {
    return NULL;
  }

  // Divide hyperedges into inner hyperedges and hyperedges containing some
  // connecting nodes
  ...
  nbr_sel_cov =
      setInitSubset(Connector, bound_edges, cov_sep_set, in_comp, cov_weights);

  // Initialize AddEdges array
  for (i = 0; i < bound_edges.size(); i++)
    if (in_comp[i])
      if (!bound_edges[i]->isHeavy()) add_edges.push_back(bound_edges[i]);
  for (i = 0; i < inner_edges.size(); i++)
    if (!inner_edges[i]->isHeavy()) add_edges.push_back(inner_edges[i]);
  if (add_edges.size() <= 0)
    writeErrorMsg("Illegal number of hyperedges.", "DetKDecomp::decomp");

  if (nbr_sel_cov >= 0) {
    do {
      // Check whether a covering hyperedge within the component was
      // selected
      add_edge = true;
      ...
      if (!add_edge || (MyK - nbr_sel_cov > 0)) {
        ...
        do {
          ...
          if (add_edge) separator->insert(add_edges[i_add]);

          SubedgeSeparatorFactory sub_sep_fac;

          do {
            // Check if selected hyperedges were already used before as
            // separator
            ...
            for (i = 0; i < partitions.size(); i++) {
              if (partitions[i].size() >= cnt_edges) {
                // writeErrorMsg("Monotonicity violated.",
                // "DetKDecomp::decomp");
                fail_sep = true;
            ...
            if (!fail_sep) {
              ...
              if (htree != nullptr) {
                ...
                if (sub_edge) {
                  for (auto he : *separator) he->setLabel(-1);
                } else {
                  for (i = 0; i < nbr_sel_cov; i++) {
                    j = cov_sep_set[i];
                    in_comp[j] ? bound_edges[j]->setLabel(-1)
                               : bound_edges[j]->setLabel(0);
                  }
                  if (add_edge) add_edges[i_add]->setLabel(-1);
                }
            ...
            if (MyBIP && htree == nullptr) {
              ...
              while (!found && (separator = sub_sep_fac.next())->size() != 0) {
                ...
                if (!covers(separator, Connector)) found = false;
              }

              // All subedge separators tried
              if (separator->size() == 0) sub_edge = false;
            }

          } while (!tle && MyBIP && sub_edge && htree == nullptr);

        } while (!tle && add_edge && (htree == nullptr) &&
                 (++i_add < add_edges.size()));
      }

    } while (!tle && (htree == nullptr) &&
             ((nbr_sel_cov = setNextSubset(Connector, bound_edges, cov_sep_set,
                                           in_comp, cov_weights)) > 0));
  }
  return htree;
}
...
/*
***Description***
The method expands pruned hypertree nodes, i.e., subgraphs which were not
decomposed but are known to be decomposable are decomposed.
...
void DetKDecomp::expandHTree(HypertreeSharedPtr &HTree) {
  ...
  while ((cut_node = HTree->getCutNode()) != nullptr) {
    ...
    Separator parent_sep(
        HyperedgeVector(parent_lambda.begin(), parent_lambda.end()));

    // Decompose subgraph
    subtree = decomp(edges, connector, cut_node->getLabel());
    if (subtree == nullptr)
      writeErrorMsg("Illegal decomposition pruning.",
                    "H_DetKDecomp::expandHTree");
...
/*
***Description***
The method builds a hypertree decomposition of a given hypergraph as described
in Gottlob and Samer: A Backtracking-Based Algorithm for Computing
Hypertree-Decompositions.
