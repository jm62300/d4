...
HypertreeSharedPtr BalKDecomp::decomp(const HyperedgeVector &Edges) {
  ...
  if ((htree = decompTrivial(Edges, VertexSet())) != nullptr) return htree;
  ...
  for (auto he : Edges)
    for (auto v : he->allVertices()) vertices.insert(v);
  ...
  while ((indices = comb.next()) != nullptr && htree == nullptr) {
    ...
    if (isBalanced(partitions, Edges.size())) {
      ...
      if (checked.find(sep_edge) == checked.end() &&
          (MyHg->getNbrOfSuperEdges() == 0 ||
           find(Edges.begin(), Edges.end(), sep_edge) == Edges.end())) {
        ...
        if (subtrees.size() > 0) {
          for (auto e : sep->allEdges()) e->setAllLabels(-1);
  ...
  if (htree == nullptr) {
    ...
    while (!bal_seps.empty() && htree == nullptr) {
      ...
      do {
        ...
        for (auto he : sep->allEdges()) he->setAllLabels(-1);
...
list<HypertreeSharedPtr> BalKDecomp::decompose(
  ...
  for (auto part : Parts) {
    failed =
        getHypergraph<HyperedgeVector>(hypergraph, succ, part.component(), Sup);
  ...
  if (htree == nullptr) subtrees.clear();

  return subtrees;
}

HyperedgeVector BalKDecomp::getNeighborEdges(
    const HyperedgeVector &Edges) const {
  HyperedgeVector neighbors;

  for (auto he : Edges) {
    if (!he->isHeavy()) neighbors.push_back(he);
...
bool BalKDecomp::isBalanced(const vector<DecompComponent> &Parts,
  ...
  if (Parts.size() > 1) {
    for (auto p : Parts)
      if (p.size() > CompSize / 2) return false;
...
template <typename T>
bool BalKDecomp::getHypergraph(HypergraphSharedPtr &Hg, bool &Succ,
                               const T &Part,
                               const SuperedgeSharedPtr &Sup) const {
  ...
  for (auto hg : sFailedHg) {
    ...
    if (hg->getNbrOfEdges() == cnt) {
      if (Sup != nullptr && !hg->hasEdge(Sup)) found = false;
      if (found && !hg->hasAllEdges(Part)) found = false;
    } else
      found = false;
  ...
  if (Sup != nullptr) Hg->insertEdge(Sup);
  Succ = false;
  return false;
}
...
void BalKDecomp::expandHTree(const HypertreeSharedPtr &HTree) {
  ...
  */
  ...
  while ((cut_node = HTree->getCutNode()) != nullptr) {
    ...
    if (!succ)
      writeErrorMsg("Hypergraph was not successfully decomposed!",
