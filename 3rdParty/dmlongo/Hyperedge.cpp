...
Hyperedge::Hyperedge(const std::string &name) : Hyperedge(++G_EdgeID, name) {}

Hyperedge::Hyperedge(const std::string &name, const VertexSet &vertices)
    : Hyperedge(++G_EdgeID, name, vertices) {}
...
void Hyperedge::add(const VertexSharedPtr &v) {
  ...
  Vertices.insert(v);
  // else
  //	writeErrorMsg("Hyperedge " + getName() + " already contains vertex with
  // id " + to_string(v->getId()), "Hyperedge::addVertex");
}
...
VertexSharedPtr Hyperedge::find(const std::string &name) {
  for (auto v : Vertices)
...
bool Hyperedge::isCoveredBy(const VertexSet &v) const {
  for (auto edge_vertex : Vertices)
    if (v.find(edge_vertex) == v.cend()) return false;
...
void Hyperedge::setAllLabels(int label) const {
  ...
  for (auto v : Vertices) v->setLabel(label);
}
