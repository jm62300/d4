...
void SubedgeSeparatorFactory::init(const HypergraphSharedPtr &hg,
                                   const HyperedgeVector &comp,
                                   const SeparatorSharedPtr &sep,
                                   const std::unique_ptr<Subedges> &subs) {
  hg->setAllLabels();

  // Label the component
  for (auto he : comp) he->setAllLabels(1);
...
SeparatorSharedPtr SubedgeSeparatorFactory::next() {
  bool inc = false;
  SeparatorSharedPtr sep = std::make_shared<Separator>();

  if (!MyInit)
    writeErrorMsg("SubedgeSeparatorFactory not initialized!",
                  "SubedgeSeparatorFactory::next()");
