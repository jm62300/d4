...
std::list<HyperedgeSharedPtr> AllSubedges;
int iSub{0};
...
void Subedges::init(const HyperedgeSharedPtr &he) {
  ...
  if (entry == MySubedges.end()) {
    ...
    for (auto e : MyHg->allEdgeNeighbors(he))
      if (!e->isHeavy()) neighbors.push_back(e);
    n = neighbors.size();
    if (n < k) k = n;
      ...
      do {
        ...
        for (i = 0; indices[i] != -1; i++)
          for (auto v : neighbors[indices[i]]->allVertices())
            if (find(he->allVertices().begin(), he->allVertices().end(), v) !=
                he->allVertices().end())
        ...
        if (!he->sameVertices(new_vertices)) {
          ...
          for (auto e : MySubedges[he])
            if (e->sameVertices(new_vertices)) new_edge = e;
...
void Subedges::init() {
  for (auto e : MyHg->allEdges()) init(e);
}

void Subedges::push_back(const HyperedgeSharedPtr &orig,
...
HyperedgeSharedPtr Subedges::get(const HyperedgeSharedPtr &orig,
                                 size_t index) const {
  try {
