...
std::list<SuperedgeSharedPtr> MySuperedges;
...
void Superedge::add(const HyperedgeVector &Edges, const VertexSet &Vertices) {
  ...
  for (auto v : Vertices) Hyperedge::add(v);
}
...
SuperedgeSharedPtr Superedge::getSuperedge(const HyperedgeVector &Edges,
                                           const VertexSet &VetComp) {
  VertexSet vertices;
  ...
  for (auto he : Edges) {
    for (auto v : he->allVertices()) {
      if (VetComp.find(v) != VetComp.end()) vertices.insert(v);
    }
  }
  ...
  for (auto se : MySuperedges)
    if (se->getNbrOfVertices() == vertices.size()) {
      found = true;
      for (auto it = se->allVertices().begin();
           it != se->allVertices().end() && found; it++)
  ...
  std::string name = "SE";
  name += std::to_string(MySuperedges.size() + 1);
...
void Superedge::setAllLabels(int label) const {
  ...
  for (auto e : Edges) e->setLabel(label);
}
...
std::ostream &operator<<(std::ostream &stream,
  ...
  for (auto v_it = super->Vertices.begin(); v_it != super->Vertices.end();) {
    stream << (*v_it)->getName();
    v_it++;
    if (v_it != super->Vertices.end()) std::cout << ",";
  }
  stream << "(";
  for (auto e_it = super->Edges.begin(); e_it != super->Edges.end();) {
    stream << (*e_it)->getName();
    e_it++;
    if (e_it != super->Edges.end()) std::cout << ",";
  }
  stream << ")";
