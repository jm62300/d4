...
void Hypertree::removeCoveredEdges(list<HyperedgeSharedPtr> &edges) const {
  ...
  for (auto child_it = MyChildren.cbegin();
       child_it != MyChildren.cend() && edges.size() > 0; child_it++)
...
void Hypertree::labelChiSets(int iLabel) {
  ...
  for (auto child : MyChildren) child->labelChiSets(iLabel);
}

void Hypertree::collectChiSets(VertexSet &vertices) const {
  for (auto v : MyChi) vertices.insert(v);
  ...
  for (auto he : MyLambda)
    if (CovNodes[he->getLabel()] == nullptr)
      CovNodes[he->getLabel()] = this;
             ...
             (MyLambda.size() < CovNodes[he->getLabel()]->getLambda().size()))
      CovNodes[he->getLabel()] = this;

  // Set the chi-set for all children
  for (auto child : MyChildren) child->selCovHTNodes(CovNodes);
}
...
void Hypertree::setChi_Conn() {
  ...
  for (auto child : MyChildren) child->labelChiSets(++i);

  if (!MyParent.expired()) {
    HypertreeSharedPtr parent = MyParent.lock();
    // Label all nodes that occur in the chi-set of the parent and of at least
    // one subtree by -1
    for (auto chi : parent->allChi())
      if (chi->getLabel() > 0) chi->setLabel(-1);
  }

  // Put all nodes in the actual chi-set that are labeled with -1
  for (auto v : MyHg->allVertices())
    if (v->getLabel() == -1) MyChi.insert(v);

  // Set the chi-sets for all children
  for (auto child : MyChildren) child->setChi_Conn();
}
...
void Hypertree::reduceLambdaTopDown() {
  ...
  iNbrOfNodes = 0;
  for (auto he : MyLambda) iNbrOfNodes += he->getNbrOfVertices();
  iNbrOfHEdges = MyLambda.size();
  if (parent != nullptr) iNbrOfHEdges += parent->MyLambda.size();
  ...
  "Hypertree::reduceLambdaTopDown"); */

  // Reset the labels of the nodes in the actual hyperedges
  for (auto he : MyLambda) {
    Lambda.insert(he);
    HEdges.insert(he);
    for (auto v : he->allVertices()) v->setLabel(0);
  }
  MyLambda.clear();

  // Label all nodes in the actual hyperedges that occur in the hyperedges of at
  // least two children by -1
  int i = 1;
  ...
  if (parent != nullptr)
    // Label all nodes in the actual hyperedges that occur in the hyperedges of
    // the parent node and at least one child node by -1
    for (auto he : parent->MyLambda) {
      HEdges.insert(he);
      for (auto v : he->allVertices())
        if (v->getLabel() > 0) v->setLabel(-1);
    }
  ...
  // Select all nodes in the actual hyperedges that are marked: they must be
  // covered
  for (auto he : Lambda) {
    for (auto v : he->allVertices())
      if (v->getLabel() == -1) Nodes.insert(v);
  }
  ...
  for (auto child : MyChildren) child->reduceLambdaTopDown();
}
...
void Hypertree::reduceLambdaBottomUp() {
  ...
  while (Child != nullptr) {
    ...
    // Compute an upper bound for the memory needed in this method
    iNbrOfNodes = 0;
    for (auto he : Child->MyLambda) iNbrOfNodes += he->getNbrOfVertices();
    iNbrOfHEdges = Child->MyLambda.size();
    for (auto ch : Child->MyChildren) iNbrOfHEdges += ch->MyLambda.size();
    ...
    if (!Child->MyParent.expired()) child_parent = Child->MyParent.lock();

    if (child_parent != nullptr)
      // Label all nodes in the actual hyperedges that occur in the hyperedges
      // of the parent node and at least one child node by -1
    ...
    for (auto he : Lambda) {
      ...
      bFinalOccurrence = true;
      if ((child_parent != nullptr) &&
          (child_parent->MyLambda.find(he) != child_parent->MyLambda.end()))
        bFinalOccurrence = false;
...
void Hypertree::writeGMLNodes(ofstream &GMLFile) const {
  ...
  for (i = 0, LambdaIter = MyLambda.begin(); LambdaIter != MyLambda.end();
       i++, LambdaIter++) {
    Lambda[i] = *LambdaIter;
  ...
  // Write lambda-set
  if (MyLambda.size() > 0) GMLFile << Lambda[0]->getName();
  ...
  /*if (MyFec) {
          ...
          // TODO we can delete all of these lines and write directly the
  vectors
...
void Hypertree::insChi(const VertexSharedPtr &Vertex) { MyChi.insert(Vertex); }
...
void Hypertree::insChild(const std::shared_ptr<Hypertree> &Child,
  ...
  if (bSetParent) Child->setParent(shared_from_this());
}
...
void Hypertree::insPointer(void *Ptr) { MyPointers.insert(Ptr); }

void Hypertree::insID(int iID) { MyIDs.insert(iID); }

void Hypertree::setCut(bool bCut) { MyCut = bCut; }

bool Hypertree::isCut() const { return MyCut; }

HypertreeSharedPtr Hypertree::getCutNode() {
  HypertreeSharedPtr HTree = nullptr;

  if (MyCut) return shared_from_this();

  // Search for cut nodes in the subtrees rooted at the child nodes
  for (auto child : MyChildren)
    if ((HTree = child->getCutNode()) != nullptr) return HTree;
...
Hypertree *Hypertree::getHTNode(void *Ptr) {
  ...
  for (auto child : MyChildren)
    if ((HTree = child->getHTNode(Ptr)) != nullptr) return HTree;
...
Hypertree *Hypertree::getHTNode(int iID) {
  ...
  if (SetIter != MyIDs.end()) return this;  // Ptr was found in the actual node

  // Search for Ptr in the subtrees rooted at the child nodes
  for (auto child : MyChildren)
    if ((HTree = child->getHTNode(iID)) != nullptr) return HTree;
...
bool Hypertree::isRoot() const { return MyParent.expired(); }
...
size_t Hypertree::getHTreeWidth() const {
  ...
  // Compute maximum between treewidths of all subtrees and the actual
  // cardinality
  for (auto child : MyChildren) {
    iTmp = child->getHTreeWidth();
    if (iTmp > iTreeWidth) iTreeWidth = iTmp;
  }
...
void Hypertree::setLabel(int iLabel) { MyLabel = iLabel; }

void Hypertree::resetLabels() {
  MyLabel = 0;

  for (auto child : MyChildren) child->resetLabels();
}

int Hypertree::setIDLabels(int iStartID) {
  MyLabel = iStartID;
  for (auto child : MyChildren) iStartID = child->setIDLabels(iStartID + 1);
...
void Hypertree::shrinkByLambda() {
  ...
  while (Child != nullptr) {
    ...
    // Remove the actual node if its lambda-set is a subset of the lambda-set of
    // one of its children
    for (ChildIter = Child->MyChildren.begin(); ChildIter != MyChildren.end();)
      if (isSubset<HyperedgeSharedPtr>(Child->MyLambda,
                                       (*ChildIter)->MyLambda)) {
        // Move the lambda-set from the child to the actual node
        for (auto he : (*ChildIter)->MyLambda) Child->MyLambda.insert(he);

        // Move the chi-set from the child to the actual node
        for (auto v : (*ChildIter)->MyChi) Child->MyChi.insert(v);

        // Move the pointers from the child to the actual node
        for (auto ptr : (*ChildIter)->MyPointers) Child->MyPointers.insert(ptr);

        // Remove the child node (its contents is now stored in the actual node)
        for (auto ch : (*ChildIter)->MyChildren) Child->insChild(ch);
        (*ChildIter)->MyChildren.clear();
        ChildIter = Child->MyChildren.erase(ChildIter);
      } else
        ++ChildIter;

    // Remove all child nodes whose lambda-sets are subsets of the lambda-set of
    // the actual node
    for (ChildIter = Child->MyChildren.begin();
         ChildIter != Child->MyChildren.end();)
      if (isSubset<HyperedgeSharedPtr>((*ChildIter)->MyLambda,
                                       Child->MyLambda)) {
        // Move the chi-set from the child to the actual node
        for (auto v : (*ChildIter)->MyChi) Child->MyChi.insert(v);
...
void Hypertree::shrinkByChi() {
  ...
  while (Child != nullptr) {
    ...
    // Remove the actual node if its chi-set is a subset of the chi-set of one
    // of its children
    for (ChildIter = Child->MyChildren.begin();
         ChildIter != Child->MyChildren.end();)
    ...
    // Remove all child nodes whose chi-sets are subsets of the chi-set of the
    // actual node
    for (ChildIter = Child->MyChildren.begin();
         ChildIter != Child->MyChildren.end();)
...
void Hypertree::swapChiLambda()
{
        ...
        for(LambdaIter=MyLambda.begin(); LambdaIter != MyLambda.end();
LambdaIter++) Tmp.insert(*LambdaIter); MyLambda.clear();
...
void Hypertree::elimCovEdges() {
  ...
  for (auto child : MyChildren) child->elimCovEdges();
}
...
void Hypertree::setChi(bool bStrict) {
  ...
  for (auto e : MyHg->allEdges()) {
    if (CovNodes[i] != nullptr)
      for (auto v : e->allVertices()) CovNodes[i]->insChi(v);
    i++;
  }
...
void Hypertree::setLambda() {
  ...
  for (auto v : MyChi) {
    Nodes.insert(v);
    for (auto he : MyHg->allVertexNeighbors(v)) HEdges.insert(he);
  }
  ...
  if (!MyLambda.empty()) {
    ...
    MyHg->setVertexLabels();
    for (auto he : MyLambda)
      for (auto v : he->allVertices()) v->setLabel(1);
    ...
    if (CovEdges2.size() < CovEdges1.size()) CovEdges1 = CovEdges2;
  }

  // Copy all covering hyperedges into the lambda-set
  for (auto he : CovEdges1) MyLambda.insert(he);

  // Set the lambda-sets for all subtrees
  for (auto child : MyChildren) child->setLambda();
}
...
HypertreeSharedPtr Hypertree::findCoverNode(const HyperedgeSharedPtr &edge) {
  ...
  if (MyLambda.find(edge) != MyLambda.end()) return shared_from_this();

  for (auto child : MyChildren)
    if ((node = child->findCoverNode(edge)) != nullptr) return node;
...
HypertreeSharedPtr Hypertree::findNodeByLambda(const HyperedgeSharedPtr &edge) {
  ...
  if (it != MyLambda.end()) return shared_from_this();

  for (auto it = MyChildren.begin(); it != MyChildren.end() && node == nullptr;
       it++)
...
VertexSharedPtr Hypertree::checkCond2(list<VertexSharedPtr> &forbidden) const {
  ...
  for (auto child : MyChildren) {
    if ((Witness = child->checkCond2(forbidden)) != nullptr) return Witness;
    for (auto v : child->allChi()) {
      if (MyChi.find(v) == MyChi.end()) forbidden.push_back(v);
    }
  }
...
std::shared_ptr<const Hypertree> Hypertree::checkCond3() const {
  ...
  for (auto he : MyLambda)
    for (auto v : he->allVertices()) lambda.insert(v);

  // Check if each vertex in MyChi is covered by MyLambda
  for (auto v : MyChi)
    if (lambda.find(v) == lambda.end()) return shared_from_this();

  // check cond 3 for all children
  for (auto child : MyChildren) {
    auto witness = child->checkCond3();
    if (witness != nullptr) return witness;
  }
