...
/*
***Description***
The method partitions a given set of hyperedges into connected components, i.e.,
into sets of hyperedges in the same component and sets of nodes connecting the
components with the separator.
...
size_t Decomp::separate(const SeparatorSharedPtr &sep,
                        const HyperedgeVector &edges,
                        std::vector<DecompComponent> &partitions) const {
  int label = 0;
  std::unordered_map<HyperedgeSharedPtr, int, NamedEntityHash> eLabels;
  std::unordered_map<VertexSharedPtr, int, NamedEntityHash> vLabels;
  ...
  for (auto &e : sep->allEdges()) {
    eLabels[e] = -1;
    for (auto &v : e->allVertices()) vLabels[v] = -1;
  }

  for (auto &he : edges)
    if (eLabels[he] == 0) {
      DecompComponent comp(std::static_pointer_cast<BaseSeparator>(sep));
      label++;
...
HypertreeSharedPtr Decomp::decompTrivial(const HyperedgeVector &edges,
                                         const VertexSet &connector) const {
  HypertreeSharedPtr htree{nullptr};
  size_t total_weight = 0;

  for (auto &e : edges) total_weight += e->getWeight();

  // Stop if the hypergraph can be decomposed into a single hypertree-node
  if (total_weight <= MyK) return getHTNode(edges, edges, connector);
  ...
  if (connector.size() == 0) {
    ...
    if (edges.size() == total_weight) {
      ...
      if (total_weight <= MyK * 2) {
        ...
        for (auto it = edges.begin(); heavy == nullptr && it != edges.end();
             it++)
          if ((*it)->isHeavy())
            heavy = std::dynamic_pointer_cast<Superedge>(*it);

        // put all other edges into the first bag
        for (auto &he : edges)
          if (he != heavy) part.push_back(he);
...
/*
***Description***
The method creates a new hypertree-node, inserts the given hyperedges into the
lambda-set, inserts the nodes of hyperedges labeled by -1 to the chi-set,
inserts the nodes in ChiConnect to the chi-set, and adds the given hypertrees as
subtrees. It is assumed that hyperedges that should be covered by the chi-set
are labeled by -1.
...
HypertreeSharedPtr Decomp::getHTNode(
    const HyperedgeVector &comp, const HyperedgeVector &lambda,
    const VertexSet &ChiConnect, const std::list<HypertreeSharedPtr> &Subtrees,
    const SuperedgeSharedPtr &Super) const {
  HypertreeSharedPtr HTree = std::make_shared<Hypertree>(MyHg);
  VertexSet vcomp;

  for (auto &e : comp)
    for (auto &v : e->allVertices()) vcomp.insert(v);
  ...
  for (auto &e : lambda) {
    ...
    for (auto &v : e->allVertices())
      if (vcomp.find(v) != vcomp.end() &&
          (Super == nullptr || Super->find(v) != nullptr))
        HTree->insChi(v);
  }

  // Insert additional chi-labels to guarantee connectedness
  for (auto &v : ChiConnect) HTree->insChi(v);
  ...
  for (auto &subtree : Subtrees)
    ...
    if (Super != nullptr) {
      ...
      if (root != nullptr) {
        root->setRoot();
        for (auto &child : root->allChildren()) HTree->insChild(child);
        root->remChildren(false);
      } else
        writeErrorMsg("Superedge not found in the subtree.",
                      "BalKDecomp::getHTNode");
    } else
...
HypertreeSharedPtr Decomp::getCutNode(int label, const HyperedgeVector &lambda,
                                      const VertexSet &ChiConnect) const {
