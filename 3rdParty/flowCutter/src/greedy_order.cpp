...
ArrayIDFunc<std::vector<int>> build_dyn_array(const ArrayIDIDFunc& tail,
  ...
  for (int i = 0; i < arc_count; ++i) neighbors[tail(i)].push_back(head(i));
...
struct NullAssign {
 public:
  NullAssign& operator=(const T&) { return *this; }
};
...
Iter3 set_union_and_remove_element(Iter1 a, Iter1 a_end, Iter2 b, Iter2 b_end,
  ...
  while (a != a_end && b != b_end) {
    if (*a < *b) {
      if (*a != remove_element1 && *a != remove_element2) *out++ = *a;
      ++a;
    } else if (*a > *b) {
      if (*b != remove_element1 && *b != remove_element2) *out++ = *b;
      ++b;
    } else if (*a == *b) {
      if (*a != remove_element1 && *a != remove_element2) *out++ = *a;
      ++b;
...
int compute_number_of_shortcuts_added_if_contracted(
    const ArrayIDFunc<std::vector<int>>& graph, int node) {
  int added = 0;
  for (int x : graph(node)) {
    std::set_difference(graph(node).begin(), graph(node).end(),
                        graph(x).begin(), graph(x).end(),
...
ArrayIDIDFunc compute_greedy_min_degree_order(const ArrayIDIDFunc& tail,
  ...
  for (int x = 0; x < node_count; ++x) q.push(x, g(x).size());
...
ArrayIDIDFunc compute_greedy_min_shortcut_order(const ArrayIDIDFunc& tail,
  ...
  for (int x = 0; x < node_count; ++x)
    q.push(x, 100 * compute_number_of_shortcuts_added_if_contracted(g, x) +
                  g(x).size());
  ...
  while (!q.empty()) {
    ...
    for (auto y : contract_node(g, x)) {
      q.push_or_set_key(
          y, 100 * compute_number_of_shortcuts_added_if_contracted(g, y) +
                 g(y).size());
    }
  }
