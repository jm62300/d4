...
struct Graph {
  Graph(Tail tail, Head head, BackArc back_arc, Capacity capacity,
        OutArc out_arc)
      : tail(std::move(tail)),
        head(std::move(head)),
  ...
  int node_count() const { return tail.image_count(); }

  int arc_count() const { return tail.preimage_count(); }
};

struct TemporaryData {
  TemporaryData() {}
  explicit TemporaryData(int node_count) : node_space(node_count) {}
  ArrayIDFunc<int> node_space;
};

template <class Tail, class Head, class BackArc, class Capacity, class OutArc>
Graph<Tail, Head, BackArc, Capacity, OutArc> make_graph(Tail tail, Head head,
                                                        BackArc back_arc,
                                                        Capacity capacity,
                                                        OutArc out_arc) {
  return {std::move(tail), std::move(head), std::move(back_arc),
          std::move(capacity), std::move(out_arc)};
}

template <class Tail, class Head, class BackArc, class OutArc>
Graph<Tail, Head, BackArc, ConstIntIDFunc<1>, OutArc> make_graph(
    Tail tail, Head head, BackArc back_arc, OutArc out_arc) {
  return {std::move(tail), std::move(head), std::move(back_arc),
          ConstIntIDFunc<1>(tail.preimage_count()), std::move(out_arc)};
}

class PseudoDepthFirstSearch {
 public:
  template <class Graph, class WasNodeSeen, class SeeNode,
            class ShouldFollowArc, class OnNewArc>
  void operator()(const Graph& graph, TemporaryData& tmp, int source_node,
                  const WasNodeSeen& was_node_seen, const SeeNode& see_node,
                  const ShouldFollowArc& should_follow_arc,
                  const OnNewArc& on_new_arc) const {
    int stack_end = 1;
...
struct UnitFlow {
  ...
  void clear() { flow.fill(1); }

  int preimage_count() const { return flow.preimage_count(); }

  template <class Graph>
  void increase(const Graph& graph, int a) {
    auto f = flow(a);
    assert((f == 0 || f == 1) &&
           "Flow is already maximum; can not be increased");
  ...
  void decrease(const Graph& graph, int a) {
    auto f = flow(a);
    assert((f == 1 || f == 2) &&
           "Flow is already minimum; can not be decreased");
    assert(flow(graph.back_arc(a)) == 2 - f && "Back arc has invalid flow");
    --f;
    flow.set(a, f);
    flow.set(graph.back_arc(a), 2 - f);
  ...
  int operator()(int a) const { return static_cast<int>(flow(a)) - 1; }

  void swap(UnitFlow& o) { flow.swap(o.flow); }
...
class BasicNodeSet {
 public:
  ...
  bool can_grow() const { return extra_node != -1; }

  template <class Graph, class SearchAlgorithm, class OnNewNode,
            class ShouldFollowArc, class OnNewArc>
  void grow(const Graph& graph, TemporaryData& tmp,
            const SearchAlgorithm& search_algo,
            const OnNewNode&
                on_new_node,  // on_new_node(x) is called for every node x. If
                              // it returns false then the search is stopped, if
                              // it returns true it continues
            const ShouldFollowArc&
                should_follow_arc,  // is called for a subset of arcs and must
                                    // say whether the arc sould be followed
            const OnNewArc& on_new_arc  // on_new_arc(xy) is called for ever arc
                                        // xy with x in the set
  ) {
    assert(can_grow());
    ...
    auto was_node_seen = [&](int x) { return inside_flag(x); };

    search_algo(graph, tmp, extra_node, was_node_seen, see_node,
  ...
  bool is_inside(int x) const { return inside_flag(x); }

  int node_count_inside() const { return node_count_inside_; }

  int max_node_count_inside() const { return inside_flag.preimage_count(); }
...
class AssimilatedNodeSet {
 ...
 public:
  template <class Graph>
  explicit AssimilatedNodeSet(const Graph& graph) : node_set(graph) {}
  ...
  bool can_grow() const { return node_set.can_grow(); }

  template <class Graph, class SearchAlgorithm, class OnNewNode,
            class ShouldFollowArc, class OnNewArc, class HasFlow>
  void grow(const Graph& graph, TemporaryData& tmp,
            const SearchAlgorithm& search_algo,
            const OnNewNode&
                on_new_node,  // on_new_node(x) is called for every node x. If
            ...
            const ShouldFollowArc&
                should_follow_arc,  // is called for a subset of arcs and must
                                    // say whether the arc sould be followed
            const OnNewArc& on_new_arc,  // on_new_arc(xy) is called for ever
                                         // arc xy with x in the set
            const HasFlow& has_flow) {
    auto my_on_new_arc = [&](int xy) {
      if (has_flow(xy)) front.push_back(xy);
      on_new_arc(xy);
    };

    node_set.grow(graph, tmp, search_algo, on_new_node, should_follow_arc,
                  my_on_new_arc);
  }

  bool is_inside(int x) const { return node_set.is_inside(x); }

  int node_count_inside() const { return node_set.node_count_inside(); }

  int max_node_count_inside() const { return node_set.max_node_count_inside(); }
  ...
  const std::vector<int>& get_cut_front() const { return front; }
...
class ReachableNodeSet {
 public:
  ...
  void reset(const AssimilatedNodeSet& other) { node_set = other.node_set; }

  void clear() { node_set.clear(); }
  ...
  void grow(const Graph& graph, TemporaryData& tmp,
    ...
    node_set.grow(graph, tmp, search_algo, on_new_node, my_should_follow_arc,
                  on_new_arc);
  }
  ...
  void forall_arcs_in_path_to(const Graph& graph, const IsSource& is_source,
                              int target, const OnNewArc& on_new_arc) {
    int x = target;
    while (!is_source(x)) {
...
struct CutterStateDump {
  BitIDFunc source_assimilated, target_assimilated, source_reachable,
      target_reachable, flow;
};
...
class BasicCutter {
 public:
  ...
  CutterStateDump dump_state() const {
    return {
        id_func(assimilated[source_side].max_node_count_inside(),
                [&](int x) { return assimilated[source_side].is_inside(x); }),
        id_func(assimilated[target_side].max_node_count_inside(),
                [&](int x) { return assimilated[target_side].is_inside(x); }),
        id_func(assimilated[source_side].max_node_count_inside(),
                [&](int x) { return reachable[source_side].is_inside(x); }),
        id_func(assimilated[target_side].max_node_count_inside(),
                [&](int x) { return reachable[target_side].is_inside(x); }),
        id_func(flow.preimage_count(), [&](int xy) { return flow(xy) != 0; })};
  }

  //! Returns true if a new cut was found. Returns false if no cut was found.
  //! False implies that no cut will be found in the future. Repeatly calling
  //! this function after it returned false does not do anything.
  ...
  bool is_cut_available() const { return cut_available; }

  template <class Graph, class ScorePierceNode>
  bool does_next_advance_increase_cut(
      const Graph& graph, const ScorePierceNode& score_pierce_node) {
    int side = get_current_cut_side();
  ...
  int get_current_cut_side() const {
    if (reachable[source_side].node_count_inside() ==
            assimilated[source_side].node_count_inside() &&
        (reachable[target_side].node_count_inside() !=
             assimilated[target_side].node_count_inside() ||
         assimilated[source_side].node_count_inside() <=
             assimilated[target_side].node_count_inside()))
      return source_side;
    else
  ...
  int get_assimilated_node_count() const {
    return assimilated[source_side].node_count_inside() +
           assimilated[target_side].node_count_inside();
  }

 private:
  template <class Graph, class ScorePierceNode>
  int select_pierce_node(const Graph& graph, int side,
                         const ScorePierceNode& score_pierce_node) {
    int pierce_node = -1;
    ...
    for (auto xy : assimilated[side].get_cut_front()) {
      ...
      if (!assimilated[1 - side].is_inside(y)) {
        int score =
            score_pierce_node(y, side, reachable[1 - side].is_inside(y));
  ...
  bool is_saturated(const Graph& graph, int direction, int xy) {
    if (direction == target_side) xy = graph.back_arc(xy);
  ...
  void check_invariants(const Graph& graph) {
    ...
    for (int side = 0; side < 2; ++side)
      assert(assimilated[side].node_count_inside() > 0 &&
             "Each side must contain at least one node");

    for (int x = 0; x < graph.node_count(); ++x)
      assert((!assimilated[source_side].is_inside(x) ||
              !assimilated[target_side].is_inside(x)) &&
    ...
    for (int side = 0; side < 2; ++side)
      for (int x = 0; x < graph.node_count(); ++x)
        if (assimilated[side].is_inside(x))
          assert(reachable[side].is_inside(x) &&
    ...
    assert(reachable[smaller_reachable_side].node_count_inside() ==
           assimilated[smaller_reachable_side].node_count_inside());
    for (int x = 0; x < graph.node_count(); ++x)
      assert(reachable[smaller_reachable_side].is_inside(x) ==
             assimilated[smaller_reachable_side].is_inside(x));
...
enum class DistanceType { no_distance, hop_distance, weighted_distance };
...
class DistanceAwareCutter {
 ...
 public:
  ...
  void init(const Graph& graph, TemporaryData& tmp,
            ...
            SourceTargetPair p, int random_seed) {
  ...
  CutterStateDump dump_state() const { return cutter.dump_state(); }
  ...
  bool advance(const Graph& graph, TemporaryData& tmp,
    ...
    auto my_score_pierce_node = [&](int x, int side,
      ...
      return score_pierce_node(x, side, causes_augmenting_path,
                               node_dist[side](x), node_dist[1 - side](x));
    };
    return cutter.advance(graph, tmp, search_algo, my_score_pierce_node);
  }

  bool is_cut_available() const { return cutter.is_cut_available(); }
  ...
  int get_current_cut_side() const { return cutter.get_current_cut_side(); }
  ...
  bool is_on_smaller_side(int x) const { return cutter.is_on_smaller_side(x); }

  bool is_empty() const { return node_dist[0].preimage_count() == 0; }
...
class MultiCutter {
 public:
  ...
  void init(const Graph& graph, TemporaryData& tmp,
            ...
            const std::vector<SourceTargetPair>& p, int random_seed,
    ...
    while (cutter_list.size() > p.size())
      cutter_list.pop_back();  // can not use resize because that requires
                               // default constructor...
    while (cutter_list.size() < p.size()) cutter_list.emplace_back(graph);
    ...
    for (int i = 0; i < (int)p.size(); ++i) {
      auto& x = cutter_list[i];
      if ((int)x.get_current_cut().size() < best_cut_size ||
    ...
    current_smaller_side_size =
        cutter_list[current_cutter_id].get_current_smaller_cut_side_size();
  }

  CutterStateDump dump_state() const {
    if (cutter_list.size() != 1)
      throw std::runtime_error(
          "Can only dump the cutter state if a single instance is run");
  ...
  bool advance(const Graph& graph, TemporaryData& tmp,
    ...
    int current_cut_size =
        cutter_list[current_cutter_id].get_current_cut().size();
    for (;;) {
          ...
          if ((int)x.get_current_cut().size() == current_cut_size) {
            ...
            if (x.advance(graph, tmp, search_algo, my_score_pierce_node)) {
              ...
              while (!x.does_next_advance_increase_cut(graph,
                ...
                if (!x.advance(graph, tmp, search_algo, my_score_pierce_node))
                  break;
                if (!should_skip_non_maximum_sides) break;
              }
            }
          }
        }
      ...
      if (next_cut_size == std::numeric_limits<int>::max()) return false;

      int best_cutter_weight = 0;
      int best_cutter_id = -1;
      for (int i = 0; i < (int)cutter_list.size(); ++i) {
        if (cutter_list[i].is_cut_available()) {
          if ((int)cutter_list[i].get_current_cut().size() == next_cut_size &&
              cutter_list[i].get_current_smaller_cut_side_size() >
                  best_cutter_weight) {
            best_cutter_id = i;
            best_cutter_weight =
                cutter_list[i].get_current_smaller_cut_side_size();
      ...
      if (best_cutter_weight <= current_smaller_side_size) continue;
  ...
  int get_current_cutter_id() const { return current_cutter_id; }
  ...
  int operator()(int x, int side, bool causes_augmenting_path, int source_dist,
    ...
    auto random_number = [&] {
      if (side == BasicCutter::source_side)
        ...
        return (hash_factor * ((unsigned)(x << 1) + 1) + hash_offset) %
               hash_modulo;
    };
    ...
    switch (config.avoid_augmenting_path) {
      case Config::AvoidAugmentingPath::avoid_and_pick_best:
        if (causes_augmenting_path) score -= 1000000000;
        break;
      ...
      case Config::AvoidAugmentingPath::avoid_and_pick_random:
        if (causes_augmenting_path) score = random_number() - 1000000000;
        break;
      default:
        assert(false);
        score = 0;
...
class SimpleCutter {
 public:
  SimpleCutter(const Graph& graph, Config config)
      : graph(graph), tmp(graph.node_count()), config(config) {}

  void init(const std::vector<SourceTargetPair>& p, int random_seed) {
    DistanceType dist_type;

    if (config.pierce_rating == Config::PierceRating::min_source_hop_dist ||
        config.pierce_rating == Config::PierceRating::max_target_hop_dist ||
        config.pierce_rating ==
            Config::PierceRating::max_target_minus_source_hop_dist)
  ...
  int get_current_cutter_id() const { return cutter.get_current_cutter_id(); }
...
std::vector<SourceTargetPair> select_random_source_target_pairs(
    int node_count, int cutter_count, int seed) {
  std::vector<SourceTargetPair> p(cutter_count);
  std::mt19937 rng(seed);
  for (auto& x : p) {
    do {
      // We do not use std::uniform_distribution because it produces difference
      // results for different compilers
