...
void kahypar_set_custom_target_block_weights(
  ...
  kahypar::Context &context =
      *reinterpret_cast<kahypar::Context *>(kahypar_context);
...
void kahypar_configure_context_from_file(kahypar_context_t *kahypar_context,
  ...
  kahypar::parseIniToContext(
      *reinterpret_cast<kahypar::Context *>(kahypar_context), ini_file_name);
}

void kahypar_set_fixed_vertices(
    kahypar_hypergraph_t *kahypar_hypergraph,
    const kahypar_partition_id_t *fixed_vertex_blocks) {
  kahypar::Hypergraph &hypergraph =
      *reinterpret_cast<kahypar::Hypergraph *>(kahypar_hypergraph);
...
kahypar_create_hypergraph_from_file(const char *file_name,
  ...
  kahypar::io::readHypergraphFile(file_name, num_hypernodes, num_hyperedges,
                                  ...
                                  &hypernode_weights);
  return reinterpret_cast<kahypar_hypergraph_t *>(new kahypar::Hypergraph(
      num_hypernodes, num_hyperedges, index_vector, edge_vector, num_blocks,
...
kahypar_create_hypergraph(const kahypar_partition_id_t num_blocks,
  ...
  return reinterpret_cast<kahypar_hypergraph_t *>(new kahypar::Hypergraph(
      num_vertices, num_hyperedges, hyperedge_indices, hyperedges, num_blocks,
...
void kahypar_improve_hypergraph_partition(
    kahypar_hypergraph_t *kahypar_hypergraph,
  ...
  ALWAYS_ASSERT(context.partition.mode == kahypar::Mode::direct_kway,
                "V-cycle refinement of input partitions is only possible in "
                "direct k-way mode");
  ASSERT(*std::max_element(input_partition,
                           input_partition + hypergraph.initialNumNodes()) ==
         num_blocks - 1);
  ASSERT(
      [&]() {
        std::unordered_set<kahypar_partition_id_t> set(
            input_partition, input_partition + hypergraph.initialNumNodes());
        LOG << V(set.size());
        for (kahypar_partition_id_t i = 0; i < num_blocks; ++i) {
  ...
  std::memcpy(improved_partition, input_partition,
              hypergraph.initialNumNodes() * sizeof(kahypar_partition_id_t));

  kahypar_partition_hypergraph(kahypar_hypergraph, num_blocks, epsilon,
                               objective, kahypar_context, improved_partition);
}

void kahypar_read_hypergraph_from_file(
  ...
  kahypar::io::readHypergraphFile(file_name, *num_vertices, *num_hyperedges,
                                  indices_ptr, hyperedges_ptr,
                                  hyperedge_weights_ptr, vertex_weights_ptr);
...
void kahypar_partition(const kahypar_hypernode_id_t num_vertices,
  ...
  kahypar::Hypergraph hypergraph(
      num_vertices, num_hyperedges, hyperedge_indices, hyperedges,
      context.partition.k, hyperedge_weights, vertex_weights);
...
void kahypar_improve_partition(
    const kahypar_hypernode_id_t num_vertices,
  ...
  ALWAYS_ASSERT(context.partition.mode == kahypar::Mode::direct_kway,
                "V-cycle refinement of input partitions is only possible in "
                "direct k-way mode");
  ASSERT(*std::max_element(input_partition, input_partition + num_vertices) ==
         num_blocks - 1);
  ASSERT(
      [&]() {
        std::unordered_set<kahypar_partition_id_t> set(
            input_partition, input_partition + num_vertices);
        LOG << V(set.size());
        for (kahypar_partition_id_t i = 0; i < num_blocks; ++i) {
          if (set.find(i) == set.end()) {
            return false;
  ...
  // use improved_partition as temporary_input_partition
  std::memcpy(improved_partition, input_partition,
              num_vertices * sizeof(kahypar_partition_id_t));

  kahypar_partition(num_vertices, num_hyperedges, epsilon, num_blocks,
                    vertex_weights, hyperedge_weights, hyperedge_indices,
                    hyperedges, objective, kahypar_context, improved_partition);
}
